...
// RSA_KEY_BITS specifies the size of the RSA key generated used by OIDC
const RSA_KEY_BITS = 2048

// RegisterHandlers : Registers all OAuth2 endpoints through http.HandleFunc
func RegisterHandlers() {
	// Set up oauth2 endpoints. You could also use gorilla/mux or any other router.
	http.HandleFunc("/o2/auth", authEndpoint)
	http.HandleFunc("/o2/token", tokenEndpoint)

	// revoke tokens
	http.HandleFunc("/o2/revoke", revokeEndpoint)
	http.HandleFunc("/o2/introspect", introspectionEndpoint)
}
